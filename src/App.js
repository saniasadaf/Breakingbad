...
function App() {
  ...
  const renderListUsingMap = () =>{
    const mappedList = data.results.map((item,idx) => <Card  name={item.name} nickname={item.name} occupation={item.occupation} birthday={item.birthday} category={item.category} char_id={item.char_id} src={item.src}/> )
   return mappedList
   }
